...
(() => {
 ...
 const show = isLoggedIn() && isAdminUser();
...
`;
 ...
 return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`;
	...
	function adminIcon() { return iconWrap(`<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>`); }
 ...
 */
 ...
 init();
	...
	window.addEventListener("jod:auth-changed", () => {
 syncAdminNavLinks();
 if (typeof window.updateNavAuth === "function") {
 try { window.updateNavAuth(); } catch (_) {}
 }
	});
	window.addEventListener("jod:auth-expired", () => {
 syncAdminNavLinks();
	});
})();
